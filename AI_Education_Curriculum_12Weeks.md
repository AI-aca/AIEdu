...
#### 🎯 [1단계 최종 실습과제]: 안티그래비티 2.0 설치/개념/출력 3대 핵심 미션
*   *실습 미션*: 다음 3가지 핵심 미션을 순서대로 하나씩 자력으로 수행하십시오.
    *   **[미션 1 - 설치]**: 안티그래비티 2.0 에디터를 다운로드하고, 무설치형 독립 실행 파일이 화면에 정상 구동되는지 확인하십시오.
    *   **[미션 2 - 개념]**: 1회차 서버 개념 영상을 토대로 내 PC 에디터와 외부 클라우드 에이전트 간의 통신 상호작용 방식을 회고하며 인프라 흐름을 이해하십시오.
    *   **[미션 3 - 출력]**: 챗 창에 `"안녕 안티그래비티"`라고 한글로 입력한 뒤, 우측 프리뷰 창에 환영 메시지가 정상 출력되는지 최종 확인하십시오. *(가장 기초적인 인프라 구동 확인 과정)*
...
*   *실습 미션*: 3~5회차 학습 내용을 바탕으로 다음 4가지 핵심 미션을 완수하십시오.
    *   **[미션 1 - 홍보카드]**: 내 학원의 이름과 전화번호, 핵심 가치 등을 포함하는 카드형 UI 요소를 프리뷰에 배치하도록 명령하십시오.
    *   **[미션 2 - 디자인]**: 5회차 영상의 UI 리디자인 기법을 활용하여 배경 색상, 테두리 둥글기, 폰트 정렬 등 디자인 레이아웃 정밀 튜닝.
    *   **[미션 3 - Alert]**: 명함 카드 내에 '전화 문의' 버튼을 추가하고, 클릭 시 안내창(Alert Popup)이 깔끔하게 작동되도록 하십시오.
    *   **[미션 4 - 로컬공유]**: 완성된 파일들이 로컬 PC의 어느 경로에 저장되었는지 탐색하여, 이 결과물 폴더를 압축 파일 등으로 타인에게 전달하여 공유할 수 있도록 준비하십시오.
...
*   *실습 미션*: 다음 3가지 핵심 미션을 완성하십시오.
    *   **[미션 1 - 초6과목할인합산]**: 단과 수강료 계산기 웹앱을 만들고, '초등학교 6학년' 대상 과목을 여러 개 선택 시 총 수강료에서 10%의 단과 결합 할인이 자동으로 합산 연산되어 계산되도록 로직을 탑재하십시오.
    *   **[미션 2 - 자가에러]**: 연산 입력창에 숫자가 아닌 문자를 고의로 입력하여 예외 연산 에러(NaN 등)를 발생시키는 오작동 상황을 만드십시오.
...
#### 🎯 [4단계 최종 실습과제]: 가상학생성적상담 DB/GAS연동 2대 핵심 미션
*   *실습 미션*: 구글 스프레드시트와 앱스 스크립트 연동을 다음 2가지 핵심 미션으로 진행하십시오.
    *   **[미션 1 - 가상학생성적상담 DB]**: 구글 스프레드시트에 '학생명/학년/점수/상담일자/상담결과'를 입력할 수 있는 테이블 컬럼 구조를 생성하고, 이를 학원 상담용 기본 데이터베이스(DB)로 기획하십시오.
    *   **[미션 2 - GAS연동 배포]**: 안티그래비티가 자동 작성해 준 구글 앱스 스크립트(GAS) 코드를 스프레드시트 에디터에 이식하고, 구글 웹앱 '새 배포' 설정을 수행하여 모바일이나 외부 디바이스에서 전 세계 어디서나 접속할 수 있는 고유 웹앱 배포 URL 링크를 런칭하십시오.
...
*   *실습 미션*: 커리큘럼의 최종 릴리즈 및 백업을 위해 다음 2가지 핵심 미션을 연속으로 수행하십시오.
    *   **[미션 1 - GitHub 배포]**: 12회차 얄코 Git 영상에서 다룬 개념을 적용하여, 안티그래비티 2.0의 Git 제어판을 활용해 GitHub 계정과 연동하고, 지난 12주간 빌드한 전체 마스터 소스코드를 깃허브 원격 저장소에 원클릭으로 커밋 및 푸시하여 안전하게 영구 저장하십시오.
    *   **[미션 2 - 모바일 실전 데이터 검증]**: 4단계에서 발급받았던 모바일 웹앱 주소로 스마트폰을 통해 직접 웹사이트에 접속한 후, 가상의 학생 성적 및 상담 결과를 전송하십시오. 구글 스프레드시트 DB에 해당 데이터 행이 누락 없이 실시간으로 즉시 기록 및 축적되는지 눈으로 전수 교차 검증하여 최종 데이터 파이프라인의 무결성을 확보하십시오.
