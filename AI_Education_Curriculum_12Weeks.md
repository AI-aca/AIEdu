...
#### 🎯 [1단계 최종 실습과제]: 안티그래비티 2.0 설치/파일 안내/출력 3대 핵심 미션
*   *실습 미션*: 다음 3가지 핵심 미션을 순서대로 하나씩 자력으로 수행하십시오.
    *   **[미션 1 - 설치 및 출력]**: 안티그래비티 2.0 에디터를 다운로드하여 컴퓨터 화면에 구동해 봅니다. 그 후, AI 비서에게 첫 인사를 건네고 환영 메시지가 정상적으로 답변 창에 출력되는지 확인해 보세요.
    *   **[미션 2 - 파일명 확인]**: 에디터 왼쪽에 들어있는 기본 웹 화면 파일(HTML), 마크다운 문서 파일(MD), 구글 앱스 스크립트 코드 파일(GS), 자바스크립트 파일(JS)의 역할이 각각 무엇인지 안티그래비티에게 직접 질문하여 가이드를 확인해 봅니다.
    *   **[미션 3 - 개념]**: 1회차 서버 개념 영상을 보며, 내 컴퓨터 에디터와 인터넷 클라우드 에이전트가 어떻게 데이터를 서로 주고받으며 동작하는지 그 원리를 가볍게 이해하고 넘어가 봅니다.
...
*   **주차 학습 내용**: 학습 노트를 펴고 영상에 나오는 20대 용어를 메모하여 안티그래비티 에이전트와 대화할 어휘 빌드업.
...
*   **주차 학습 내용**: 안티그래비티 3탄 UI 디자인 영상을 시청하고, 학원 홍보 전단지나 캡처 이미지를 에디터에 업로드하여 3단 랜딩 페이지로 스타일을 모방하고 복제하도록 명령하는 조종법을 익힙니다.
...
*   *실습 미션*: 구글 스프레드시트와 앱스 스크립트 연동을 다음 2가지 핵심 미션으로 진행하십시오.
    ...
    *   **[미션 3 - 연동]**: 안티그래비티가 자동 작성해 준 구글 앱스 스크립트(GAS) 코드를 스프레드시트 에디터에 이식하고, 구글 웹앱 '새 배포' 설정을 수행하여 다지인된 화면에서 학생의 성적 및 상담 정보를 입력했을 때 데이터베이스에 저장될 수 있는 구조를 구축하시오. (프론트엔드 및 백엔드 연동)
...
*   *실습 미션*: 커리큘럼의 최종 릴리즈 및 백업을 위해 다음 2가지 핵심 미션을 연속으로 수행하십시오.
    *   **[미션 1 - GitHub 배포]**: 12회차 얄코 Git 영상의 개념을 살려, 안티그래비티 2.0의 Git 메뉴를 활용해 내 GitHub 계정과 연동해 봅니다. 4단계에서 빌드한 성적 및 상담 입력 웹앱 소스코드를 깃허브 원격 저장소에 원클릭으로 커밋/푸시하여 인터넷상에서 누구나 들어올 수 있는 고유 웹 주소(URL)를 생성해 보세요.
